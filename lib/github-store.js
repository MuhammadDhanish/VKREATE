...
const TTL_MS = 1000; // 1-second in-memory cache TTL for instant API data freshness
...
async function ghWrite(file, mutateFn) {
  ...
  if (!GITHUB_TOKEN) {
    ...
    const normalized = normalizeSchema(file, updated);
    let writeOk = false;
    for (let i = 0; i < 3; i++) {
      try {
        const localPath = path.join(__dirname, '..', file);
        fs.writeFileSync(localPath, JSON.stringify(normalized, null, 2), 'utf8');
        writeOk = true;
        break;
      } catch (e) {
    ...
    TTL_CACHE[file] = { data: normalized, timestamp: Date.now() };
    return { success: writeOk, data: normalized };
  }
